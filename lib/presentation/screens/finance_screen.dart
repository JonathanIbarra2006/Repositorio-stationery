...
class _FinanceScreenState extends ConsumerState<FinanceScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        // --- LOGO A LA IZQUIERDA ---
        centerTitle: false, // Esto mueve el contenido a la izquierda
        title: Image.asset('assets/images/logo.png', height: 35), // Tamaño discreto y elegante
        actions: [
          IconButton(
  ...
  void _showModal(BuildContext context, WidgetRef ref, TransactionType tipo) {
    ...
    double monto = 0;
    String desc = '', cat = '';
    showModalBottomSheet(context: context, isScrollControlled: true, builder: (ctx) => Padding(padding: EdgeInsets.only(bottom: MediaQuery.of(ctx).viewInsets.bottom, left: 16, right: 16, top: 16), child: SingleChildScrollView(child: Form(key: formKey, child: Column(mainAxisSize: MainAxisSize.min, children: [
      Text(tipo == TransactionType.ingreso ? 'Ingreso' : 'Gasto', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
      TextFormField(decoration: const InputDecoration(labelText: 'Monto'), keyboardType: TextInputType.number, onSaved: (v) => monto = double.parse(v!)),
      DropdownButtonFormField(items: ['Ventas', 'Servicios', 'Proveedores', 'Otros'].map((e) => DropdownMenuItem(value: e, child: Text(e))).toList(), onChanged: (v) => cat = v.toString(), decoration: const InputDecoration(labelText: 'Categoría')),
      TextFormField(decoration: const InputDecoration(labelText: 'Descripción'), onSaved: (v) => desc = v!),
      ElevatedButton(onPressed: () { if(formKey.currentState!.validate()){ formKey.currentState!.save(); ref.read(transactionRepoProvider).addTransaction(AppTransaction(id: const Uuid().v4(), tipo: tipo, monto: monto, fecha: DateTime.now(), descripcion: desc, categoria: cat)); ref.invalidate(transactionsProvider); Navigator.pop(ctx); } }, child: const Text('Guardar')),
